...
const DEFAULT_CHUNK_SIZE_RANGE: std::ops::Range<usize> = 500..1000;
...
pub fn chunk_text(text: &str, prefix: &str) -> Result<Vec<String>, ChunkerError> {
    ...
    let mut chunks: Vec<String> = splitter.chunks(text)
        .map(|s| s.to_string())
        .collect();
